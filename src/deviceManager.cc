...
DeviceManager::DeviceManager(const vector<DiskInfo> v_diskInfo, bool isSlave) {
    ...
    // debug_error("zc***********v_diskInfo[0] fd %d\n", v_diskInfo[0].fd);
    for (DiskInfo diskInfo : v_diskInfo) {
        ...
        // debug_error("zc***********_diskInfo[%d] fd %d\n", diskInfo.diskId, _diskInfo[diskInfo.diskId].fd);
    ...
    if (cm.enabledVLogMode() || _isSlave) {
        ...
        if (isWrite) {
            if (useFS) {
                ...
                // if (_diskInfo.at(diskId).fd < 0) {
                //     debug_error("zc***********Invalid file descriptor for disk %d fd %d\n", diskId, _diskInfo.at(diskId).fd);
                // }
...
offset_t DeviceManager::accessDataOnDisk(segment_id_t segmentId, segment_offset_t startingOffset, segment_len_t accessLength, unsigned char *buf, bool isWrite) {
    ...
    if (sepSegmentFiles) {
        ...
        // debug_error("zc***********buf = %p diskOffset = %llu accessLength = %llu\n", buf, diskOffset, accessLength);
...
len_t DeviceManager::accessSegmentFile(segment_id_t segmentId, unsigned char *buf, segment_offset_t startingOffset, segment_len_t writeLength, bool isWrite) {
    ...
    len_t writeSize = accessFile(fd, buf, startingOffset, writeLength, isWrite, /* isCircular = */ cm.enabledVLogMode() || _isSlave, segmentId);
...
len_t DeviceManager::accessFile(FILE *fd, unsigned char *buf, segment_offset_t startingOffset, segment_len_t writeLength, bool isWrite, bool isCircular, segment_id_t segmentId) {
    ...
    if (isCircular) {
        ...
        // // 打开一个文件流用于写入日志
        // std::ofstream testFile("test_log.txt", std::ios::app);  // 使用追加模式
        // if (testFile.is_open()) {
        //     // 写入读/写操作的信息
        //     testFile << "Segment ID: " << segmentId << "\n"; // 记录 segmentId
        //     testFile << "Operation: " << (isWrite ? "Write" : "Read") << "\n";
        //     testFile << "Access Length: " << accessLength << "\n";
        //     testFile << "Buffer Content (Hex): ";
        //     for (size_t i = 0; i < KEY_SIZE; ++i) {
        //         testFile << std::hex << std::setfill('0') << std::setw(2) << static_cast<unsigned int>(buf[i]) << " ";
        //     }
        //     testFile << "\n\n";  // 添加一些空行以便区分不同的条目
        //     testFile.close();  // 关闭文件流
        // } else {
        //     std::cerr << "Unable to open test log file." << std::endl;
        // }
