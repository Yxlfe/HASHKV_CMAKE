...
StatsRecorder::StatsRecorder(){
    ...
    _setTimeHistogram = 0;
    hdr_init(/* min = */ 1, /* max = */ (int64_t) 100 * 1000 * 1000 *1000, /* s.f. = */ 3, &_setTimeHistogram);
    ...
    _scanTimeHistogram = 0;
    hdr_init(/* min = */ 1, /* max = */ (int64_t) 100 * 1000 * 1000 *1000, /* s.f. = */ 3, &_scanTimeHistogram);
    _deleteTimeHistogram = 0;
    hdr_init(/* min = */ 1, /* max = */ (int64_t) 100 * 1000 * 1000 *1000, /* s.f. = */ 3, &_deleteTimeHistogram);
}
...
#define PRINT_FULL(_NAME_, _TYPE_, _SUM_) \
    ...
    fprintf(stdout, "%-24s %14ld\n", "Set latency 99-th-%:", hdr_value_at_percentile(_setTimeHistogram, 99.0));
    ...
    fprintf(stdout, "%-24s %14ld\n", "Update latency 99-th-%:", hdr_value_at_percentile(_updateTimeHistogram, 99.0));
    ...
    fprintf(stdout, "%-24s %14ld\n", "Get latency 99-th-%:", hdr_value_at_percentile(_getTimeHistogram, 99.0));
    ...
    fprintf(stdout, "%-24s %14ld\n", "Scan latency 99-th-%:", hdr_value_at_percentile(_scanTimeHistogram, 99.0));
    for (auto h : _scanByLenSizeHistogram) {
       fprintf(stdout, "%-24s %llu:\n", "- Scan Len of size", h.first);
       fprintf(stdout, "%-24s %14.3lf\n", "  - mean:", hdr_mean(h.second));
       fprintf(stdout, "%-24s %14.3lf\n",   "  - stddev:", hdr_stddev(h.second));
       fprintf(stdout, "%-24s %14ld\n",   "  - min:", hdr_min(h.second));
       fprintf(stdout, "%-24s %14ld\n",   "  - max:", hdr_max(h.second));
       ...
       fprintf(stdout, "%-24s %14ld\n",   "  - 99.99-th-%:", hdr_value_at_percentile(h.second, 99.99));
       fprintf(stdout, "%-24s %14ld\n",   "  - 100-th-%:", hdr_value_at_percentile(h.second, 100.0));
       fprintf(stderr, "%-24s [%llu] %14ld\n", "Scan latency 95-th-%:", h.first, hdr_value_at_percentile(h.second, 95.0));
       fprintf(stderr, "%-24s [%llu] %14ld\n", "Scan latency 100-th-%:", h.first, hdr_value_at_percentile(h.second, 100.0));
    }

    fprintf(stdout,"------------------------- DELETE Request --------------------------------------\n");
    PRINT_FULL("Delete Time"            , DELETE                            , time[DELETE]);
    fprintf(stdout, "%-24s %14ld\n", "Delete latency 99-th-%:", hdr_value_at_percentile(_deleteTimeHistogram, 99.0));
...
#undef PRINT_GC_BUCKETS
    ...
    */
    ...
    free(_setTimeHistogram);
    _setTimeHistogram = 0;
    free(_updateTimeHistogram);
    _updateTimeHistogram = 0;
    free(_getTimeHistogram);
    _getTimeHistogram = 0;
    free(_scanTimeHistogram);
    _scanTimeHistogram = 0;
    free(_deleteTimeHistogram);
    _deleteTimeHistogram = 0;

    for (auto h : _setByValueSizeHistogram) {
        free(h.second);
    }
    for (auto h : _updateByValueSizeHistogram) {
        free(h.second);
...
unsigned long long StatsRecorder::timeProcess (StatsType stat, struct timeval &start_time, size_t diff, size_t count, unsigned long long valueSize) {
    ...
    if (stat == StatsType::SET) {
        hdr_record_value(_setTimeHistogram, ret);
        if (valueSize > 0) {
            if (_setByValueSizeHistogram.count(valueSize) == 0) {
                _setByValueSizeHistogram[valueSize] = 0;
                hdr_init(/* min = */ 1, /* max = */ (int64_t) 100 * 1000 * 1000 *1000, /* s.f. = */ 3, &_setByValueSizeHistogram[valueSize]);
            }
            hdr_record_value(_setByValueSizeHistogram.at(valueSize), ret);
        }
    } 
    else if (stat == StatsType::UPDATE) {
    ...
    else if (stat == StatsType::SCAN) {
        hdr_record_value(_scanTimeHistogram, ret);
        if (valueSize > 0) {
            if (_scanByLenSizeHistogram.count(valueSize) == 0) {
                _scanByLenSizeHistogram[valueSize] = 0;
                hdr_init(/* min = */ 1, /* max = */ (int64_t) 100 * 1000 * 1000 *1000, /* s.f. = */ 3, &_scanByLenSizeHistogram[valueSize]);
            }
            hdr_record_value(_scanByLenSizeHistogram.at(valueSize), ret);
        }
    }
    else if (stat == StatsType::DELETE) {
        hdr_record_value(_deleteTimeHistogram, ret);
    }
