...
KvServer * KvServer::getInstance(DeviceManager *deviceManager) {
    static KvServer _instance(deviceManager);
    return &_instance;
}
...
KvServer::KvServer(DeviceManager *deviceManager, bool freeDeviceManager) {
    // devices
    if (deviceManager) {
    ...
    _freeDeviceManager = freeDeviceManager;
...
void KvServer::getRangeValues(char *startingKey, uint32_t numKeys, std::vector<char*> &keys, std::vector<char*> &values, std::vector<len_t> &valueSize) {
    ...
    for (uint32_t i = 0; i < numKeys && kit->isValid(); i++, kit->next()) {
        ...
        // printf("zc////// count = %llu keys[%u] = ", test_count, i + 1);
        // test_count++;
        // for (int j = 0; j < KEY_SIZE; ++j) {
        //     printf("%02x ", static_cast<unsigned char>(keys[i][j]));
        // }
        // printf("\n\n");
