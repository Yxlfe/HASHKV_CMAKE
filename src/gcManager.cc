...
size_t GCManager::gcOneGroup(group_id_t groupId, GCMode &finalGCMode, bool needsLockCentralizedReservedPool, len_t originBytes, group_id_t* reportGroupId) {
    ...
    // debug_error("zc***mainSegmentSize = %lu logSegmentSize = %lu\n", mainSegmentSize, logSegmentSize);
    for (size_t gcount = 0, cur = 0; gcount < 1 + logSegments.size(); gcount++) {
        ...
        if (useMmap) {
            ...
            bool ret = Segment::init(segment, cid, csize, false);
            // debug_error("zc***Segment::init ret = %d\n", ret);
        }
    ...
    // debug_error("zc***logSegments.size() = %zu\n", logSegments.size());
    // debug_error("zc***gcMode = %d\n", gcMode);
    ...
    // debug_error("zc***_valueManager->_segmentReservedByGroup.count(groupId) = %zu\n", _valueManager->_segmentReservedByGroup.count(groupId));
    ...
    // len_t count_keyCount = 0;
    // for (auto &it : keyCount) {
    //     count_keyCount++;
    //     len_t t_valueSize = 0;
    //     memcpy(&t_valueSize, it.first + KEY_SIZE, sizeof(len_t));
    //     debug_error("zc***t_valueSize = %lu count_keyCount = %lu\n", t_valueSize, count_keyCount);
    // }
...
size_t GCManager::gcSegment(group_id_t groupId, Segment &segment, std::unordered_map<unsigned char *, std::pair<int, ValueLocation>, hashKey, equalKey> &keyCount, len_t total, bool isRemove, size_t reservedPos, int gcMode, size_t *validBytes) {
    ...
    // debug_error("zc***total = %lu bytesScanned = %zu reservedPos = %zu gcMode = %d\n", total, bytesScanned, reservedPos, gcMode);
...
segment_len_t GCManager::gcKvPair(group_id_t groupId, Segment *segment, segment_len_t scanned, std::unordered_map<unsigned char *, std::pair<int, ValueLocation>, hashKey, equalKey> &keyCount, bool isRemove, size_t reservedPos, int gcMode, size_t *validBytes) {
    ...
    if(nullptr == kc.first)
    {
        debug_error("zc***Segment::getData return nullptr","");
    }
