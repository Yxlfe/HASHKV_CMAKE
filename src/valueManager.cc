...
bool ValueManager::getValueFromDisk (const char *keyStr, ValueLocation readValueLoc, char *&valueStr, len_t &valueSize) {
    ...
    if (cm.useSlave() && readValueLoc.segmentId == cm.getNumSegment() && !_isSlave) {
        ...
        // static uint64_t test_count = 1;
        // static uint32_t test_index = 1;
        // // 使用debug_error打印keyStr的内容
        // std::cout << "zc////// count = " << test_count << " keyStr[" << test_index << "]: ";

        // for (int i = 0; i < KEY_SIZE; ++i) {
        //     std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<unsigned int>(keyStr[i]) << " ";
        // }
        // std::cout << std::dec << "\n";

        // // 使用debug_error打印valueStr的内容
        // std::cout << "zc////// valueStr[" << test_index << "]: ";
        // for (int i = 0; i < KEY_SIZE; ++i) {
        //     std::cout << std::hex << std::setfill('0') << std::setw(2) << static_cast<unsigned int>(valueStr[i]) << " ";
        // }
        // std::cout << std::dec << "\n\n";
        ...
        // static uint32_t test_index = 1;
        // // 使用debug_error打印keyStr的内容
        // printf("zc////// count = %llu keyStr[%u]: ", test_count, test_index);


        // for (int i = 0; i < KEY_SIZE; ++i) {
        //     printf("%02x ", static_cast<unsigned char>(keyStr[i]));
        // }
        // printf("\n");

        // // 使用debug_error打印valueStr的内容
        // printf("zc////// valueStr[%u]: ", test_index);
        // for (int i = 0; i < KEY_SIZE; ++i) {
        //     printf("%02x ", static_cast<unsigned char>(valueStr[i]));
        // }
        // printf("\n\n");
...
#define FLUSH_SEGMENT_BUFFER(_THRD_) do { \
    ...
    for (auto group = _centralizedReservedPool[flushingPoolIndex].groupsInPool.begin();
        ...
        for (auto update = _centralizedReservedPool[flushingPoolIndex].segmentsInPool.at(mainSegmentId).second.begin();
            ...
            // debug_error("zc***********valueSize = %llu RECORD_SIZE = %lld\n", valueSize, RECORD_SIZE);
            ...
            // debug_error("isReservedOverflow = %d\n",isReservedOverflow);
